...
private:
	...
	Node<_Kty, _Ty>* _erase(Node<_Kty, _Ty>* subtree, _Kty key) {
		if (key > subtree->nodePair.first) subtree->right = _erase(subtree->right, key);
			...
			subtree = new Node<_Kty, _Ty>(min(temp.right)->nodePair.first, min(temp.right)->nodePair.second);
...
public:
	...
	_Ty next(_Kty key) {
		...
		if (lastBigger) return lastBigger->nodePair.second;
	}
	...
	_Ty prev(_Kty key) {
		...
		if (lastSmaller) return lastSmaller->nodePair.second;
	}
